...
- **Restart-safe idempotent state machine** (Codex C1): the embedding impl creates the remote batch
  BEFORE recording its id (`app-memory-backfill-provider-batch.ts:45`) → crash orphans paid work. v1
  needs a durable submission-intent/idempotency key, immutable page snapshot + content-hash, and
  transactional apply/cursor. Result download/JSONL parse errors must be handled (not just poll errors).
...
- `gantry observer setup` = one wizard step (extend `cli/setup-flow.ts`, clack, template
