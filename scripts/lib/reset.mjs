...
// persona with a prior open lead + chat context. (Ported from reset-test-records.mjs,
// schema-qualified for the per-opportunity model.) Talks only to the shared DB.
import { randomUUID } from 'node:crypto';

export async function resetTestData(client, phones) {
  const convIds = phones.map((p) => `conversation:wa:${p}`);
  await client.query(`set lock_timeout = '10s'`);
  await client.query(`set statement_timeout = '30s'`);

  await client.query(`delete from boondi_crm.boondi_business_records where phone = any($1)`, [phones]);
  await client.query(`delete from boondi_crm.boondi_digest_cursor where conversation_id = any($1)`, [convIds]).catch(() => {});
  await client.query(`delete from gantry.memory_items where user_id = any($1)`, [phones]).catch(() => {});
  // Digests reference sessions (FK); drop them before the sessions.
  await client
    .query(`delete from gantry.agent_session_digests where agent_session_id in (select id from gantry.agent_sessions where conversation_id = any($1))`, [convIds])
    .catch(() => {});
  ...
  for (const tbl of ['runtime_events', 'jobs', 'agent_runs', 'agent_sessions']) {
    await client.query(`delete from gantry.${tbl} where conversation_id = any($1)`, [convIds]).catch((e) => {
      console.error(`  [reset] gantry.${tbl}: ${e.message}`);
    });
  }
  // Conversations cascade to messages -> message_parts and participants.
  await client.query(`delete from gantry.conversations where id = any($1)`, [convIds]);
}
...
export async function seedReturning(client, phone) {
  ...
  await client.query(
    `insert into boondi_crm.boondi_business_records
  ...
  const seedMsg = async (suffix, direction, trust, sender, deliveryStatus, ts, text) => {
    ...
    await client.query(`insert into gantry.message_parts (message_id, ordinal, kind, payload_json) values ($1,0,'text',$2)`, [id, JSON.stringify({ kind: 'text', text })]);
  };
