...
- Runtime diagnostic routes such as `/v1/runtime/workers` should stay read-only
  and runtime-generic. Keep them behind existing read scopes unless the data
  grants new authority, and add focused route plus OpenAPI inventory tests in
  the same change.
