...
const OPERATING_GUIDANCE_BLOCK = [
  ...
  '- Use request_access with target.kind=capability when a reviewed capability exists. If the source is missing, request source install/connect/attach. If the source exists but the action is unreviewed, refresh source inventory and request review.',
  '- Use request_access target.kind=capability for durable reviewed access; target.kind=run_command is the only raw fallback and must be scoped, with temporaryOnly=true for transient command needs.',
  ...
  '- request_access target.kind=run_command is only for scoped RunCommand fallback when no reviewed capability fits. Browser, admin, provider, and channel access require reviewed capability ids.',
