...
describe('group CLI commands', () => {
  it('adds and reads a non-telegram group', async () => {
    const { runGroupCommand } = await import('./group.js');
    const suffix = Date.now().toString(36);
    const jid = `dc:team-room-${suffix}`;

    expect(
      await runGroupCommand(runtimeHome, [
        'add',
        jid,
        '--name',
        'Discord Team',
    ...
    expect(await runGroupCommand(runtimeHome, ['info', jid])).toBe(0);
  ...
  it('updates and disables trigger mode', async () => {
    ...
    expect(await runGroupCommand(runtimeHome, ['trigger', jid, '@Bot'])).toBe(
      0,
    );
    expect(await runGroupCommand(runtimeHome, ['trigger', jid, '--off'])).toBe(
      0,
    );
  ...
  it('rejects folder-style selectors in group add', async () => {
    const { runGroupCommand } = await import('./group.js');
    expect(
      await runGroupCommand(runtimeHome, ['add', 'team_ops', '--name', 'Team']),
    ).toBe(1);
  });
  ...
  it('resolves numeric selector as folder when it exists', async () => {
    ...
    const infoSpy = vi
      .spyOn(console, 'log')
      .mockImplementation(() => undefined);
    expect(await runGroupCommand(runtimeHome, ['info', '123'])).toBe(0);
  ...
  it('requires --yes for remove in non-interactive mode', async () => {
    ...
    expect(
      await runGroupCommand(runtimeHome, ['add', jid, '--name', 'To Remove']),
    ).toBe(0);

    expect(await runGroupCommand(runtimeHome, ['remove', jid])).toBe(1);
    expect(await runGroupCommand(runtimeHome, ['remove', jid, '--yes'])).toBe(
      0,
    );
    expect(await runGroupCommand(runtimeHome, ['info', jid])).toBe(1);
  });
});
