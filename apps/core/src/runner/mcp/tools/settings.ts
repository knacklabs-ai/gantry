...
export function registerSettingsTools(server: McpServer): void {
  server.tool(
    ...
    'Main-agent-only: read the current local settings.yaml desired state before requesting local MyClaw configuration changes.',
    {},
    async () => {
  ...
  server.tool(
    ...
    'Main-agent-only: request a reviewed update to local settings.yaml. Provide a complete replacement settings.yaml document; the host validates it and writes it only after approval.',
    {
      replacementYaml: z
        .string()
