...
- Keep Claude SDK Bash commands sandboxed with `sandbox.network.allowLocalBinding` so approved CLIs can use the Gantry loopback egress proxy instead of bypassing the sandbox. Prefix approved Bash commands from `toolNetworkEnv`, including `GODEBUG=netdns=go`, so Go CLIs use Go DNS resolution and common Python/curl/Node tools receive the same proxy and neutral CA aliases. On macOS, also enable `sandbox.enableWeakerNetworkIsolation` so Go-based CLIs such as `gh`, `gcloud`, `terraform`, and business CLIs can reach `com.apple.trustd.agent` for TLS certificate verification. These are sandbox transport settings only; they must not grant durable `RunCommand(...)` authority or expose broker proxy/provider credentials to tools.
