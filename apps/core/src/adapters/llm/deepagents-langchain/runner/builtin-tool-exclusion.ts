...
// DeepAgents 1.10.2 always bakes the `write_todos` (todo list) middleware and
// the `task` (subagent spawner) middleware into the graph at createDeepAgent
// time (see node_modules/deepagents/dist/index.cjs `createDeepAgent`, lines
// ~8188-8208). There is NO config switch to omit them. createDeepAgent itself
// uses exactly this pattern to drop tools it wants excluded (its private
// `_ToolExclusionMiddleware`): a wrapModelCall middleware that filters
// `request.tools` before the model sees them. We append our own via the public
// `middleware` param so the model can never call `task` or `write_todos`:
