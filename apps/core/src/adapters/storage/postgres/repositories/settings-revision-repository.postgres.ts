  ...
  implements SettingsRevisionRepository
{
  ...
  async appendSettingsRevision(input: {
    ...
    // Allocate the next revision against the current max and let the
    // (app_id, revision) unique key serialize concurrent appends. A losing
    // append retries against the new max rather than overwriting.
