...
4. Update `.codex/architecture-map.json` only when the desired ownership model changes, not to hide current debt.
5. Run `python3 .codex/scripts/check_architecture.py` and `python3 .codex/scripts/check_task_completion.py` before final handoff when possible.
