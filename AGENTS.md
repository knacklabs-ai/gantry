...
- A terminally failed live channel turn must explicitly persist the processed message cursor before queue settlement. Earlier attempts may roll back for retry; the final acknowledged failure must not leave the same stored message eligible for another model run.
...
- Delegated task terminal receipts must be emitted only after the terminal task transition is durable. Keep `task.updated` payloads provider- and domain-neutral (`taskId`, optional `taskKey`, kind, status) so caller applications can enforce their own recovery gates.
...
- Application-managed provider credentials must use typed SDK credential-admin
  operations and Gantry's encrypted credential store. Reconciliation may
  compare redacted field fingerprints but must never read credentials back.
...
- Keep the Node SDK aligned with existing app-scoped agent administration routes; application packages must use SDK `agents.list`/`agents.create` rather than raw HTTP or direct Gantry database writes.
- Do not reject session `response_schema` solely from the configured runtime: the Anthropic worker lane supports provider-native structured output. Keep unsupported worker/engine combinations fail-closed in pre-spawn admission after model routing resolves the engine.
- Worker-lane caller-resolved tools must support both jobs and live session runs. Authorize live tool names and session identity from the host-created per-run sandbox policy; never trust tool definitions supplied by model-controlled IPC payloads or require a `jobId` for an otherwise valid live run.
- Live caller-resolved-tool runtime events must preserve the originating application correlation id through the host-created run policy so the caller can resolve the interaction to the exact submitted turn.
