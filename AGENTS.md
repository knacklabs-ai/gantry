...
- Model selection must go through the provider-neutral catalog and friendly aliases. Current live vocabulary is `modelAlias`, read-only effective `agentEngine`, `responseFamily` (`anthropic` or `openai`), diagnostic `modelRoute`, read-only `executionProviderId`, and `credentialProfileRef`; the current parser/API/CLI still reject writable `agentHarness` and `agent_harness`. The target harness-selection slice uses durable `agentHarness` (`auto`, `anthropic_sdk`, or `deepagents`) and the `settings.yaml` key `agent_harness`; use that vocabulary only in planned strategy docs or when implementing that slice. OpenRouter is its own provider on the DeepAgents/OpenAI-compatible lane, not an Anthropic-family alias. In the target contract, `auto` preserves provider-derived behavior (Claude -> `anthropic_sdk`; OpenAI/OpenRouter/Bedrock/Vertex/future OpenAI-compatible providers -> `deepagents`), while explicit `anthropic_sdk` or `deepagents` is user intent and must fail before runner spawn when the selected model is incompatible. Do not accept raw provider model IDs at user/API/job/MCP boundaries unless they are registered aliases; job defaults inherit interactive defaults before falling back to system `opus`, and jobs and conversations inherit the bound agent harness only after the harness-selection slice lands instead of selecting a job- or conversation-level harness. Do not reintroduce legacy Claude model registries or runner-ID defaults in public config paths.
