...
- Agent-facing guidance must not mention a generic `Workflow` tool. There is no such Gantry tool; agents should use the mounted Gantry tools such as `todo_update`, `send_message`, `ask_user_question`, `render_*`, scheduler tools, or skill request tools depending on the job.
