...
on the same box, and the **only** thing the public internet sees is your admin
...
```
   ...
   │  (Interakt webhook on core: NOT exposed — see §9 to enable)      │
   └─────────────────────────────────────────────────────────────────┘
```
...
product behaviour, zero WhatsApp plumbing. Flip to real WhatsApp later (§9).
...
| Elastic IP | Allocate + associate | A **stable** public IP — needed for the DNS record now and the Interakt webhook later. |
...
**DNS:** point a subdomain (e.g. `admin.boondi.example.com`) at the Elastic IP
with an `A` record. Caddy needs this resolving before it can issue a cert.
...
# Node 24 (matches prod) via nvm or nodesource
...
> and is **uncommitted**. The box can only run what it can pull. Commit + push that
> branch (or `rsync` your working tree up) before anything here works.

```bash
sudo mkdir -p /opt/boondi && sudo chown $USER /opt/boondi
cd /opt/boondi
git clone -b scaling-architecture <your-gantry-repo-url> Agent.Gantry
cd Agent.Gantry
npm install
...
# Scoped JSON key is the clean form:
...
> `POSTGRES_PASSWORD` for the container is read by docker-compose from `.env` too —
> set it to the same `<STRONG_PW>` you used in `GANTRY_DATABASE_URL`.
...
```bash
# 'gantry' from source = the CLI entrypoint:
...
```bash
node dist/cli/index.js settings validate
```
...
This mirrors your `scripts/boondi-runtime-stack.sh` launch order, **productionized**:
no dry-run-test scaffolding, control bound to localhost, and pm2 for
restart-on-crash + start-on-boot. Create `/opt/boondi/ecosystem.config.js`:
...
| core, mcp-shopify, mcp-crm, admin | pm2 | ✅ pm2 relaunches any process that exits | ✅ via `pm2 startup` + `pm2 save` (run once) |

pm2 restarts a crashed process automatically — that's its default behaviour. The
one-time `pm2 startup` (run the command it prints) + `pm2 save` snapshots the set
so it returns after a reboot. Verify it actually works:
...
Optional crash-loop guard (per app in the ecosystem file): `max_restarts: 10`,
`min_uptime: 10000`, `restart_delay: 3000` — so a misconfigured process backs off
...
> four-process stack, pm2 (one supervisor for everything) is simpler. Reach for
> `gantry service` only if you want fully native systemd and don't mind writing three
> more unit files for the MCP servers + admin.

---

## 9. Caddy — HTTPS for the admin dashboard only

`/etc/caddy/Caddyfile`:

```caddy
admin.boondi.example.com {
    encode gzip
    reverse_proxy 127.0.0.1:3000
    # Optional belt-and-suspenders on top of your dashboard's own login:
    # basicauth { tester $2a$14$<bcrypt-hash> }
}
```

```bash
sudo systemctl reload caddy   # auto-issues a Let's Encrypt cert on first hit
```

That's the entire public footprint. Visit `https://admin.boondi.example.com`,
log in with your dashboard's auth, drive a test conversation, watch the agent
respond. **UAT is live.**
...
1. Add a route to the Caddyfile so only the webhook path reaches core:
   ```caddy
   hooks.boondi.example.com {
       reverse_proxy /v1/ingresses/* 127.0.0.1:4710
   }
   ```
2. Register the Interakt ingress secret in Gantry (signed ingress record) and set
   the Interakt webhook URL to `https://hooks.boondi.example.com/v1/ingresses/...`.
...
10 min** — so this path must be stable before you point real traffic at it. Keep
it out of UAT until you're ready.

---

## 11. Gotchas (recap)

1. **Commit `scaling-architecture` first** — the box runs published/pulled code, and
   your two-knob + background-isolation work isn't committed.
2. **Real Anthropic API key, not personal OAuth** — §6. A shared 5-hour window under
   client load = throttling + ugly latency.
3. **Direct Postgres only** — work dispatch uses LISTEN/NOTIFY + advisory locks; a
   transaction-mode pooler (PgBouncer / Supabase `:6543`) silently breaks it.
   Postgres-in-Docker-on-the-box is a direct connection, so you're fine.
4. **Admin auth is yours** — Gantry exposes nothing public here; the dashboard's login
   is the perimeter. Don't expose it bare.
5. **This box = your first prod node** — it's on AWS, on EC2, exactly the README deploy
   model. When you add the scale-out layer, snapshot this into an AMI/launch template.
```